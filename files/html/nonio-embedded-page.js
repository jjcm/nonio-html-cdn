...
let nonioFrame = {
  ...
  init: () => {
    console.log('initializing the frame resizer')
    /* 
  ...
  */
  reportHeight: (height) => {
    ...
    var data = { page: window.location.pathname, height: height }
    nonioFrame.port?.postMessage(data)
    //window.postMessage('iframeSizeEvent: hello, world', '*')
  ...
  portListener: (event) => {
    if(event.ports){
      nonioFrame.port = event.ports[0]
      nonioFrame.reportHeight()
    }
  }
}

window.addEventListener('message', nonioFrame.portListener)
